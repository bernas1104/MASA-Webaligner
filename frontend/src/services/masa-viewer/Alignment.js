...
export default class Alignment {
    ...
    _gaps = new GapList(SEQUENCES_NUMBER);
    ...
    _createGapSequence = () => {
        ...
        while((c0 >= 0 && c0 < this._gaps[0].length) || (c1 >= 0 && c1 < this._gaps[1].length)) {
            ...
            if(c1 >= 0 && c1 < this._gaps[1].length){
                gap1 = new GapSequence({ second: { gap: this._gaps[1][c1], i, j, dir: dirI,
                    gapType: GapSequence.GapType.SEQUENCE_1 } });
            }

            if(c0 >= 0 && c0 < this._gaps[0].length){
                gap0 = new GapSequence({ second: { gap: this._gaps[0][c0], i, j, dir: dirJ,
