...
const FetchStageIResultsService = require(
    '../../../src/services/FetchStageIResultsService'
);
...
describe('Service responsible for retrieving the .bin files from the server', () => {
    ...
    beforeAll(async () => {
        ...
        execSync(
            ...
            '-1 -d ' + path.join(resultPath, `${path.parse(s0file.file).name}-${path.parse(s1file.file).name}`) +
            ' > /dev/null 2>&1'
        );
    });

    it('should return the best score information', async () => {
        const bestScore = await fetchStageIResultsService.execute({ id });

        expect(bestScore).toMatchObject({
            bestScore: expect.any(Number),
            bestPosition: expect.arrayContaining([
                expect.any(Number),
                expect.any(Number),
            ])
        });
    ...
    it('should throw an error if the alignment does not exist', async () => {
        try {
            await fetchStageIResultsService.execute({ id: uuid() });
        } catch (err) {
